...
def enforce_public_link_view(ip: str, limits: RateLimits, locale: str) -> None:
    if not limits.enabled:
        return
    enforce_checks(
        [
            (f"public_link:ip:{ip}", limits.public_link_ip, WINDOW_HOUR),
            ("public_link:global", limits.public_link_global, WINDOW_HOUR),
        ],
        locale,
    )


def enforce_public_pin_unlock(ip: str, limits: RateLimits, locale: str) -> None:
    if not limits.enabled:
        return
    enforce_checks(
        [(f"public_pin:ip:{ip}", limits.public_pin_ip, WINDOW_HOUR)],
        locale,
    )
