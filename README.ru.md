...
2. Под instance admin: Instance → воркеры → добавить ноду:
   ...
   - Для **capture** (опционально): проверить подключение и выбрать connectors ноды (`jitsi`, `telemost`, `zoom`, … из `/health` воркера, подмножество разрешённых на инстансе)
   - `weight` / `enabled` по необходимости
3. Instance → settings → **Сервисные модели**: defaults ASR/диаризации из объединения зарегистрированных transcribe-воркеров и default LLM для summarize — из имён, которые отдают включённые summarize-воркеры (`GET /health`: `model`, `llm_model` или `llm`).
4. Пользователи могут переопределить модели транскрибации и summarize в **Profile** (опционально). Выбранные модели фиксируются на каждой новой задаче.
5. **Только capture (опционально):** Instance → settings — включить capture и разрешить connectors (каталог включает id с воркеров: `jitsi`, `telemost`, …). Для **Jitsi** org admin задаёт **host** встречи (hostname + опционально JWT) в Org → capture — без привязки host → worker; hub выбирает любую включённую capture-ноду с `jitsi` (перевыбор при нехватке ёмкости). **Telemost** и другие разрешённые connectors не требуют карт host на org.
6. Опциональное **имя бота** в capture: default org (Org → capture), override пользователя (Profile) или `bot_display_name` на import/capture для одной job.
7. Пользователи хаба URL и токены воркеров не видят. Хаб копирует результат в свою БД и затем делает `DELETE` задачи на воркере.

Удаление или отключение воркера, который остался единственным источником пары моделей транскрибации, модели summarize или единственной capture-ноды для connector, может блокировать, пока **queued/running capture** ссылаются на эту ноду. Remediation (`capture_worker_id`) переносит такие задачи на другой включённый capture-воркер с тем же connector; строки Jitsi host org **не** привязаны к воркерам. Удаление без remediation всё равно снимает ноду и очищает `worker_id` у затронутых capture (running возвращаются в `queued`).
