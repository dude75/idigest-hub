...
Удаление или отключение воркера, который остался единственным источником пары моделей транскрибации, модели summarize или привязки Jitsi host организации, сначала просит замену. Хаб переписывает defaults инстанса, переопределения пользователей, задачи в очереди и карты Jitsi. Удаление без замены всё равно снимает ноду и очищает эти ссылки (карты Jitsi удаляются; capture-задачи этой ноды возвращаются в очередь).
