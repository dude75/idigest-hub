...
from tests.conftest import ADMIN_EMAIL, ADMIN_PASSWORD, err_code, login, logout, open_db, setup_admin
...
def test_reset_request_rate_limit_by_email(client: TestClient, monkeypatch):
    setup_admin(client)
    ...
    client.patch(
        ...
        json={
            ...
            "rate_limit_reset_email": 2,
            "rate_limit_reset_ip": 0,
            "rate_limit_reset_global": 0,
        },
    )
    logout(client)
    reset_rate_limiter()
    monkeypatch.setattr("app.routers.auth.send_mail", lambda *_args, **_kwargs: None)

    for _ in range(2):
        response = client.post("/api/v1/auth/password/reset/request", json={"email": ADMIN_EMAIL})
        assert response.status_code == 200

    blocked = client.post("/api/v1/auth/password/reset/request", json={"email": ADMIN_EMAIL})
    assert blocked.status_code == 429
    assert err_code(blocked) == "rate_limited"
