...
def test_parse_byte_range():
    ...
    assert _parse_byte_range("bytes=0-1999", 1000) == (0, 999)
    assert _parse_byte_range("bytes=1000-", 1000) is None
    assert _parse_byte_range("bytes=0,100-200", 1000) is None


@pytest.mark.asyncio
async def test_s3_download_response_range(monkeypatch):
    ...
    client.get_object.return_value = {"Body": _Body(payload[2:6])}

    with patch("boto3.client", return_value=client):
        backend = S3StorageBackend(get_settings())
        ref = "s3://test-bucket/uploads/a1/original.mp3"

        full = backend.download_response(ref, "clip.mp3", download=False)
        assert full.status_code == 200
        assert full.headers["accept-ranges"] == "bytes"
        assert full.headers["content-length"] == str(len(payload))
        client.get_object.assert_called_with(Bucket="test-bucket", Key="uploads/a1/original.mp3")

        client.get_object.reset_mock()
        partial = backend.download_response(
            ref, "clip.mp3", download=False, range_header="bytes=2-5"
        )
        ...
        client.get_object.assert_called_with(
            Bucket="test-bucket",
            Key="uploads/a1/original.mp3",
            Range="bytes=2-5",
        )

        invalid = backend.download_response(
            ref, "clip.mp3", download=False, range_header="bytes=20-30"
        )
