...
| ---- | ---------------- | ------ | ------ | ------ |
| Login | 30 / min | 60 / min | 500 / min | 1 min |
| Signup | 10 / min | 20 / min | 100 / min | 1 min |
...
Per-IP rate limits then use the real client IP from those headers. Per-email / per-user limits work regardless. If `TRUSTED_PROXIES` is unset and the proxy does not pass real IPs, all users share one IP for per-IP limits. Set a limit to **`0`** in Instance → Settings to disable that bucket.
