...
Default: `request.client.host` (TCP peer). With empty `TRUSTED_PROXIES` (default), `X-Forwarded-For` / `X-Real-IP` are ignored — safe when the hub is directly reachable. Set `TRUSTED_PROXIES` to the proxy IPs/CIDRs the hub sees (e.g. `127.0.0.1,::1` with nginx on the same host) and configure the proxy headers; see [`deploy/nginx/idigest-hub.conf.example`](../../../deploy/nginx/idigest-hub.conf.example).

Per-IP limits are enabled by default (e.g. login 60/min, password reset 3/hour per email + 10/hour per IP). Set a limit to **`0`** in Instance → Settings to disable that bucket. Public summary link views and PIN unlocks have separate IP/global buckets.

## Audit log

`audit_log` table records sensitive actions (setup, wallet changes, org create/delete, wipes, impersonation, summary edits, public link policy, `crypto.dek.create`, `crypto.reencrypt.*`). Instance admin **Security → Audit** lists entries via paginated API with CSV export; older rows remain queryable in DB.
