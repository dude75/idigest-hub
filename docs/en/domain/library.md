...
| Control | Who |
| ------- | --- |
| Create / replace link | Summary owner |
| View link metadata | Owner or org_admin |
| Revoke | Owner or org_admin |
| List all org links | **Public links** page — members see own links; org_admin sees all |
| Org-wide revoke | Owner (own) or org_admin (any) via `/org/public-links/{id}` |

Optional **PIN** (4–6 digits) and **expiry** (`expires_in_days`, default 7 on create). URL shape: `{public_url}/public/summary/{token}`. PIN unlock sets a short-lived cookie (30 min).

When org disables public links or Public URL is cleared, existing links stop working (`not_found` for guests). Deleting the summary revokes the link.
