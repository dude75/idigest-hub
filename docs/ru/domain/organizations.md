...
org admin создаёт пользователей с email, паролем, ролью (`org_admin` | `org_member`), locale. Новый пользователь добавляется в **ту же org**. UI может сгенерировать пароль; сброс возвращает одноразовый временный пароль в модалке.

### Создание instance admin (`POST /orgs`)

Instance admin создаёт командную org в **Instance → Organizations**:

1. Имя org, tariff (любой неархивный), email и пароль начального `org_admin`
2. Опционально `is_personal` (информационно, по умолчанию `false`)
3. Начальный баланс = `signup_credit` тарифа (или `0` при unlimited)
4. У admin `must_change_password=true`

Audit: `org.create`.

### Удаление instance admin (`POST /orgs/{org_id}/delete`)

Каскадное удаление org и всех данных: участники (кроме instance admin), артефакты, задачи, skills, shares, public links, usage events. Активные задачи отменяются или помечаются `skip_persist`. Тело: `{ "confirm_name": "<точное имя org>" }`. Audit: `org.delete`.

Instance admin может скрыть org из списка (`POST /orgs/{id}/hide`, `/unhide`) — только UI, данные не меняются.
...
| Сброс пароля админом | `POST /org/users/{id}/reset-password` | Случайный пароль; ответ содержит `password` один раз; `must_change_password=true`; отзывает sessions + API tokens |
