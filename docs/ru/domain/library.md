...
| Контроль | Кто |
| -------- | --- |
| Создать / заменить ссылку | Владелец summary |
| Посмотреть метаданные ссылки | Владелец или org_admin |
| Отозвать | Владелец или org_admin |
| Список ссылок org | Страница **Public links** — member видит свои; org_admin — все |
| Org-wide revoke | Владелец (свои) или org_admin (любые) через `/org/public-links/{id}` |

Опционально **PIN** (4–6 цифр) и **срок** (`expires_in_days`, по умолчанию 7 при создании). URL: `{public_url}/public/summary/{token}`. Разблокировка PIN ставит короткоживущую cookie (30 мин).

Если org отключил public links или сброшен Public URL, ссылки перестают работать (`not_found` для гостей). Удаление summary отзывает ссылку.

Audit: `summary.public_link.create`, `summary.public_link.revoke`, `summary.public_link.view`, `org.public_links_policy`.
